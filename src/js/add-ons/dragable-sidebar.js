(function() {
  ...
  chrome.storage.sync.get(['reorder_sidebar'], (result) => {
    ...
    const ORDER_KEY = 'SIDEBAR_ORDER_OVERRIDE';
    ...
    chrome.storage.sync.get([ORDER_KEY], (result) => {
      ...
      if (DEBUG) console.debug('DEBUG: Sidebar order restored from chrome.storage.sync:', savedOrder);
      }
    });
    ...
    function saveOrder() {
      ...
      try {
      chrome.storage.sync.set({ [ORDER_KEY]: JSON.stringify(order) });
      } catch (e) {
